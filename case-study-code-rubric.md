...
### 11.1 The one rule that overrides everything else

**All plots must appear in the RStudio plot pane. Never open a separate
graphics window.** External windows (the kind that pop up on screen and say
"R Graphics: Device N") clutter the session, cannot be navigated with the
plot pane's Back/Forward arrows, and are closed when the session ends.

The two functions that create external windows — `dev.new()` and any direct
call to a file device like `svg(path)` or `pdf(path)` before drawing — are
**prohibited** in case study scripts. Never call them, even inside a helper
function.

### 11.2 ggplot figures

Every ggplot figure follows this exact three-line pattern, which matches the
canonical example in `roaches_ch24_analysis.R`:
...
- `print(fig_N_M)` draws to the active RStudio plot pane. It must come
  **before** `ggsave()`.
- `device = svg` (the bare name, not the string `"svg"`) avoids the svglite
  dependency.
- Never assign the plot to a variable and then call `ggsave()` without
  `print()` first — the plot pane will be blank.

### 11.3 Base-R figures

For chapters that use base R graphics (`par()`, `plot()`, `lines()`, `curve()`,
etc.), draw to the active plot pane first, then copy to an SVG file with
`dev.copy()`:

```r
par(mar = c(3, 3, 2, 1), mgp = c(1.7, .5, 0), tck = -.02)
plot(...)           # draws to RStudio plot pane (the active device)
lines(...)
text(...)
dev.copy(svg, "figs/Fig-N.M.svg", width = <W>, height = <H>)
dev.off()           # closes the copy device only; pane stays open
```

`dev.copy()` copies the current plot pane contents to the named file device
and `dev.off()` closes that file device. The RStudio pane is untouched — it
remains the active device for the next plot.

When many base-R figures follow the same structure, a `save_fig()` helper
keeps the pattern DRY:
...
# Usage:
save_fig("figs/Fig-N.M.svg", width = 6, height = 4, {
  par(mar = c(3, 3, 2, 1), mgp = c(1.7, .5, 0), tck = -.02)
  plot(...)
  lines(...)
})
...
### 11.4 What never to do

| Prohibited | Why | Use instead |
|---|---|---|
| `dev.new(...)` | Opens an external graphics window | Remove it; draw directly to the pane |
| `svg("path"); ...; dev.off()` | Routes drawing to a file, skips the pane | `draw; dev.copy(svg, "path"); dev.off()` |
| `pdf("path"); ...; dev.off()` | Same | Same pattern as above |
| `x11()` / `windows()` / `quartz()` | External window | Remove; pane is always available |
| `ggsave()` without a prior `print()` | Plot pane is blank | `print(fig); ggsave(...)` |

### 11.5 Figure titles and axis labels

Figure titles must match book captions exactly. If the book does not provide
a caption, construct one as `"Figure N.M: [brief description]"`.

Apply `scale_x_continuous(breaks = ...)` on any ggplot whose x-axis covers a
...
- [ ] No `dev.new()`, `svg(path)`, `pdf(path)`, `x11()`, `windows()`, or `quartz()` calls — all prohibited
- [ ] ggplot figures: `print(fig)` before `ggsave()` — pane then file, in that order
- [ ] Base-R figures: draw directly, then `dev.copy(svg, path); dev.off()` — never `svg(path); draw; dev.off()`
- [ ] Every figure: `print()` + `ggsave()` (ggplot) or `dev.copy()` (base-R) to `figs/Fig-N.M.svg`
