// Model 8: proportional errors with distance_tolerance AND overshot as parameters
data {
  int J;
  array[J] int n;
  vector[J] x;
...
transformed parameters {
  ...
  vector[J] p_distance = Phi((distance_tolerance - overshot) ./ ((x + overshot)*sigma_distance)) - Phi(-overshot ./ ((x + overshot)*sigma_distance));
  vector[J] p = p_angle .* p_distance .* (1 - epsilon);
}
model {
  [sigma_angle, sigma_distance] ~ normal(0, 1);
  epsilon ~ exponential(1/sigma_epsilon);
  // log-normal priors from Broadie (2018) expert knowledge
  distance_tolerance ~ lognormal(log(3), 0.2);
  overshot ~ lognormal(log(1), 0.2);
  y ~ binomial(n, p);
}
