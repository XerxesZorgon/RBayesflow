...
functions {
  real integrand(real z, real notused, array[] real theta, array[] real X_i, array[] int y_i) {
...
model {
  ...
  for (n in 1:N_games) {
    target += (dif[n] == 0) ?
      log(integrate_1d(integrand, -0.5, 0,
          append_array(append_array({sigma_y}, {a[team_1[n]] - a[team_2[n]]}), {df}),
          {0}, // not used, but an empty array not allowed
          {0}, // not used, but an empty array not allowed
          1e-5) +
        integrate_1d(integrand, 0, 0.5,
          append_array(append_array({sigma_y}, {a[team_1[n]] - a[team_2[n]]}), {df}),
          {0}, // not used, but an empty array not allowed
          {0}, // not used, but an empty array not allowed
          1e-5)) :
      log(integrate_1d(integrand, dif[n]-0.5, dif[n]+0.5,
          append_array(append_array({sigma_y}, {a[team_1[n]] - a[team_2[n]]}), {df}),
          {0}, // not used, but an empty array not allowed
          {0}, // not used, but an empty array not allowed
          1e-5)); // integrate_1d_reltol));
  }
}
  ...
  for (n in 1:N_games) {
    ...
    log_lik[n] = (dif[n] == 0) ?
      ...
      log(integrate_1d(integrand, dif[n]-0.5, dif[n]+0.5,
          ...
          {0}, // not used, but an empty array not allowed
          {0}, // not used, but an empty array not allowed
          1e-6)); // integrate_1d_reltol));
  }
  y_rep_original_scale = y_rep .* abs(y_rep);
  lprior = normal_lpdf(b | 0, 1) + normal_lpdf(sigma_a | 0, 1) + normal_lpdf(sigma_y | 0, 1);
}
